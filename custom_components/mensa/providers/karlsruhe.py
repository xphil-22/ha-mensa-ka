"""Karlsruhe provider: async GraphQL client for the Karlsruher Mensen API (api.mensa-ka.de).
...
async def _post(session: aiohttp.ClientSession, query: str) -> dict[str, Any]:
...
class KarlsruheProvider:
    ...
    async def async_get_meal_plans(
        ...
        Returns a mapping of canteen id to a list of `MealDay`, one per entry
        in `days`, in the same order.
        """
